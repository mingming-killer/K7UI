 ...
 * 
 * @author harism (custom by hmm@dw.gdbbk.com)
 */
public class CurlMesh {
	...
	public synchronized boolean isFlipTexture() {
		return mFlipTexture;
	}
	...
	public synchronized void onDrawFrame(GL10 gl) {
		...
		if (DRAW_TEXTURE && mTexturePage.getTexturesChanged()) {
			...
			// TODO: by hmm 
			// now we don't create tmp bitmap for texture, it will slow down the render speed.
			//texture.recycle();
			...
			// TODO: by hmm
			// we take careful the texture page bitmap resource outside, so don't recycle bitmap here.
			// and we bitmap size is matched the curl page, so don't reset when texture is changed.
			//mTexturePage.recycle();
			mTexturePage.touchTexture();
			//reset();
		}
	...
	public synchronized void recycle() {
		...
		if (null != mArrDropShadowVertices) mArrDropShadowVertices.clear();
		if (null != mArrIntersections) mArrIntersections.clear();
		if (null != mArrOutputVertices) mArrOutputVertices.clear();
		if (null != mArrRotatedVertices) mArrRotatedVertices.clear();
		if (null != mArrScanLines) mArrScanLines.clear();
		if (null != mArrSelfShadowVertices) mArrSelfShadowVertices.clear();
		if (null != mArrTempShadowVertices) mArrTempShadowVertices.clear();
		if (null != mArrTempVertices) mArrTempShadowVertices.clear();

		if (null != mBufColors) mBufColors.clear();
		if (null != mBufCurlPositionLines) mBufCurlPositionLines.clear();
		if (null != mBufShadowColors) mBufShadowColors.clear();
		if (null != mBufShadowVertices) mBufShadowVertices.clear();
		if (null != mBufTexCoords) mBufTexCoords.clear();
		if (null != mBufVertices) mBufVertices.clear();
	}
