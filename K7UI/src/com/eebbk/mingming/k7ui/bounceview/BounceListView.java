...
/**
 * 
 * 带 Over Scroll 回弹效果的 {@link ListView} </br>
 * 
 * </br>
 * 稍稍修改下系统自带的 {@link ListView} 就可以实现，因为系统已经实现大部分框架了，
 * 只不过故意屏蔽了而已。
 * 
...
public class BounceListView extends ListView {
	...
	private final static String TAG = "BounceListView"; 
	...
	private void init(Context context, AttributeSet attrs, int defStyle) {
		if (null != attrs) {
			TypedArray a = context.obtainStyledAttributes(attrs,
					R.styleable.BounceListView, defStyle, 0);

			mMaxOverScrollY = (int) a.getDimension(
					R.styleable.BounceListView_k7uiblvMaxOverScrollY, 
